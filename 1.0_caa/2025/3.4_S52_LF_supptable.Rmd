...
pc<- catch.sam %>% filter(month !=1)  %>%  ggplot(aes(x=month, y=catch, fill=prov)) +geom_bar(stat="identity") +facet_wrap(~year) +
    scale_fill_viridis_d(name="") +
    scale_x_continuous(breaks=seq(2,12,2))
pc + labs(x="Mois | Month", y="Captures | Catch (t)")
pc + labs(x="Month", y="Catch (t)")
pc + labs(x="Mois", y="Captures (t)") 
