...
# Catch-at-age: by period
    
# SET-UP
```{r markdown, echo=F}
library(knitr)
opts_chunk$set(echo = T, collapse = T, fig.align = "center", fig.width = 9, fig.height = 6, warning=F)
options(width = 140)
```    
...
```{r caa period, message = F, fig.height = 6,fig.width = 8, warning=F}
catch.diffused.periodR <- paste0(dir.rdat, "catch.diffused.period.Rdata")
caa.periodR <- paste0(dir.rdat, "caa.period.Rdata")

these.period <- c(2,3,4)

if(!all(file.exists(c(catch.diffused.periodR,caa.periodR)))){
    # step 1) get samples
    
    catch.diffused.period <- ldply(these.period,function(x){
        print(x)
        
        years <- my.year[my.year>=1976 & my.year %in% unique(lf.caa[lf.caa$period==x,'year'])] 
        
        lf.period <- lf.caa[lf.caa$period==x & !is.na(lf.caa$period) & lf.caa$year %in% years,]
        al.period <- bio.caa[bio.caa$period==x & !is.na(bio.caa$period) & bio.caa$year %in% years,]
        ca.period <- catch.caa[catch.caa$period==x & !is.na(catch.caa$period) & catch.caa$year %in% years,]
        
        catch.diffused <- get.samples(catch=ca.period,
                                      lf=lf.period, 
                                      al=al.period,
    ...
    save(catch.diffused.period, file =  catch.diffused.periodR)
    
    caa.period <- ddply(catch.diffused.period,c('period'),function(x){x[is.na(x)]<-0;get.caa(x,plus=10)})
    
    save(caa.period, file =  caa.periodR)
}else{ 
    load(caa.periodR)
...
check <- ddply(catch.diffused.period,c('id','period'),summarise,
...
check <- reshape2::melt(check,id=c('id','period'))
ggplot(check,aes(x=value))+geom_histogram(bins=30)+facet_grid(period~variable,scale='free')  # period and period can only rarely be matched
```

### total

```{r caa check total, fig.height = 3,fig.width = 10}
tot <- ddply(catch.caa[catch.caa$period %in% these.period,],c('year','period'),summarise,catch=sum(catch))
ggplot(caa.period,aes(x=year))+
...
ggplot(caa.period,aes(x=year,y=age,size=caan))+
    geom_point(alpha=0.8)+ scale_size(range = c(1,10))+
    facet_wrap(~period,ncol=1)+
    scale_y_continuous(breaks=min(caa.period$age):max(caa.period$age))
```

## CAAN rel
```{r caan rel, message = F, fig.height = 6,fig.width = 8}
df <- caa.period[!is.na(caa.period$caan),]
df <- ddply(df,c('year','period'),mutate,caan.rel=caan/max(caan))
...
m <- acast(df[df$period==3,],age~year,value.var = 'caan.rel')
a <- cohort(m)
...
ggplot(caa.period,aes(x=year,y=age,size=caaw))+
    geom_point(alpha=0.8)+ scale_size(range = c(1,10))+
    facet_wrap(~period,ncol=1)+
    scale_y_continuous(breaks=min(caa.period$age):max(caa.period$age))
```
...
ggplot(caa.period,aes(x=year,y=waa,group=age))+
